...
for(i in 1:nrow(lineup.2014)){ 
  ...
  for(j in 1:5){ 
    ...
    index<-intersect(grep(pattern = first, x = str_replace_all(string=as.character(player.final[,1]), pattern=" ", repl=""),
                          ignore.case = T),grep(pattern = last, x = str_replace_all(string=as.character(player.final[,1]), pattern=" ", repl=""),ignore.case = T))
    l<-as.character(player.final[index,4])==as.character(lineup.2014[i,3])
    if(length(index)>0&&j==1){first.row<-player.final[index[1],]
                         row<-first.row
      }else if(length(index)>0&&j!=1){mid<-player.final[index[1],]
                               row<-cbind(row,mid,row.names = NULL)
        }else{row<-cbind(row,k,row.names = NULL)
          }
    }
  row<-row[1,]
  if(i==1){dts<-row
...
final.dts<-dts[complete.cases(dts),]
lineup.2014<-lineup.2014[complete.cases(dts),]

#write.csv(final.dts, file = "2014data.csv",row.names=FALSE, na="")
