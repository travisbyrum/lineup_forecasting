...
set.seed(17)
library(gbm)
finaldata <- read.csv("~/lineup_forecasting/data/final_data.csv", stringsAsFactors = F) #reading in data
lineups<- read.csv("~/lineup_forecasting/data/lineups.csv", stringsAsFactors = F)
#reading in data
winexpectation <- read.csv("~/lineup_forecasting/data/win_expectation2015.csv", stringsAsFactors = F)
#reading in win_expectation
...
df_rapm <- df_rapm %>% mutate(wins = winexpectation$x, Pos = as.factor(Pos), Pos.1 = as.factor(Pos.1), 
